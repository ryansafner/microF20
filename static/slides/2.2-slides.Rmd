...
- It's actually much easier to break this into *2 stages*, though it's possible to do it all at once. See today's [class notes](/class/2.1-class) page for an example.
]
]
...
- Combinations *below* the curve yield .hired[less output]; on a .red[lower curve]
  - $E < A = B = C$
]
...
- .hi[Marginal Rate of Technical Substitution (MRTS)]: rate at which firm trades off one input for another to *yield the same output*
...
- See proof in [today's class notes](/class/2.2-class)
...
- Let $C$ be the .hi[total cost] of using inputs $(l,k)$ at current input prices $(w,r)$ to produce $q$ units of output:
...
- **Slope**: market-rate of .hi-purple[tradeoff] between $l$ and $k$

- .hi-purple[Relative price] of $l$ or .hi-purple[opportunity cost] of $l$:
