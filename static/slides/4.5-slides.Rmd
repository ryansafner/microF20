---
title: "4.5 — Imperfect Competition"
...
- .hi[Monopolistic competition]: hybrid of monopoly and competition, where **each firm has _some_ market power**
  - Each firm faces own downward-sloping demand
  - Firms are price-searchers

- But, the industry has .hi-purple[free entry and exit] (no barriers)
...
- Each firm faces a downward-sloping demand curve
  - Implies some market power

- A .hi-blue[“residual” demand] for each firm’s products
...
mc_plot<-ggplot(data.frame(x=c(0,12)), aes(x=x))+
  ...
  labs(x = "Quantity (q)",
       y = "Price (p)",
       title = "Market for This Firm’s Product")+
...
- Residual demand for each firm’s product will:
  - *decrease* (more production by other firms)
  - become more *elastic* (more substitutes)
until...
...
- .hi-purple[Long run equilibrium]: firms earn $\pi=0$ where $p=AC(q)$<sup>.magenta[†]</sup>
...
- $p_c = AC(q)_{min}$, .hi-orange[productively efficient]
...
- $p_m = AC(q)$
  - but not $AC_{min}$, .hi-orange[productive inefficiency]
...
<sup>.magenta[‡]</sup> A cartel is $n$ firms that *act* as if the market were a monopoly, but each gets (for simplicity)  $\frac{1}{n}$ of the total profits.]
