---
title: "4.1 — Modeling Market Power"
...
.pull-left[
.smallest[
- In a competitive market, we modeled firms as .hi[“price-takers”] since there were so many of them selling identical products
  - Had to charge market price, could choose optimal $q^*$ to maximize $\pi$

- .hi-purple[Marginal cost pricing], which is .hi-purple[allocatively efficient] for *society*
  - $p=MC$
  - $MSB=MSC$

- In the long-run, .hi[free entry and exit] caused prices to equal (average & marginal) costs and pushed economic profits to zero
]
]
...
- Start with simple assumption of a *single* seller: .hi[monopoly]
...
.pull-left[
.smallest[
- A firm with market power is a .hi-purple[“price-maker”]
  - Can choose quantity $q^*$ *and* price $p^*$

- We can also call it a .hi-purple[“price-searcher”]
  - Assuming the same objective as before, firms with market power must **search** for $(q^*,p^*)$ that **maximizes** $\pi$
...
- .hi-turquoise[Monopolists *only* produce where demand is elastic, with positive MR(q)!]
  - See appendix in [today’s class page](/class/) for a proof
...
- .hi[Market power] defined as firm(s)’ ability to raise .hi-purple[mark up] $p>MC(q)$
...
.footnote[See [today's class notes](/class/4.1-class) for the derivation.]
...
- $\color{#0047AB}{MR(q)}>\color{#D7250E}{MC(q)}$
...
- But monopolist faces *entire* .blue[market demand]
  - Can charge as high as consumers are WTP .blue[Market Demand]
