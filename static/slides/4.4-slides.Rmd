...
.tiny[
Note: If these past few slides have sounded familiar, this is the [exact same process](https://microf20.classes.ryansafner.com/slides/4.1-slides#14) by which we derived a *monopolist*’s marginal *revenue* curve!
]
]
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  scale_y_continuous(breaks=c(4.625,6),
                     limits=c(0,10),
                     expand=c(0,0),
                     labels=c(expression(w[c]),expression(w[m])))+
  labs(x = "Labor (L)",
       y = "Wage (w)")+
  theme_classic(base_family = "Fira Sans Condensed", base_size=20)
