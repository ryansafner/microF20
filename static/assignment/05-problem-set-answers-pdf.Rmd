...
1. **In your own words, what do (1) allocative efficiency, (2) productive efficiency, and (3) Pareto efficiency mean? Under what three conditions are markets efficient?**

---

In equilibrium, markets allocate resources to their highest valued uses (allocative efficiency) by allowing the most possible voluntary exchanges. All firms are producing up to the point where marginal benefit (price) equals marginal cost. Another way to understand this is that consumer surplus (the value created for consumers from an exchange) and producer surplus (the value created for producers from an exchange) are maximized in equilibrium.

In equilibrium, competitive markets are also productively efficient, as all firms produce at the minimum of their average cost curve, using the least-cost technologies.

An allocation of resources is Pareto efficient when there are no possible Pareto improvements, that is, when there are no changes that would make at least one person better off without making at least one person worse off. Voluntary exchange is a Pareto improvement, moving resources to a higher valued use. Recall that resources "start in the wrong place," since people find opportunities for Pareto-improving exchange that make both parties better off (moving resources to higher-valued uses). In equilibrium, all possible trades have taken place, and under current market conditions, there are no more possible trades that would make at least one party better off, and no party worse off: we have reached Pareto efficiency (where there are no more possible Pareto improvements). 
...
pc<-ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  geom_label(aes(x=4.5,y=demand_firm(4.5)), color = "blue", label="Demand", size = 4)+
    stat_function(fun=mc_cc, geom="line", size=2, color = "red")+
  geom_label(aes(x=3.5,y=mc_cc(3.5)), color = "red", label="MC(q)", size = 4)+
    stat_function(fun=ac_cc, geom="line", size=2, color = "orange")+
  geom_label(aes(x=4.5,y=ac_cc(4.5)), color = "orange", label="MC(q)", size = 4)+
...
mkt<-ggplot(data.frame(x=c(0,10)), aes(x=x))+
  geom_polygon(data=cs_1, aes(x=x,y=y), fill="blue", alpha=0.5)+
  geom_polygon(data=ps_1, aes(x=x,y=y), fill="red", alpha=0.5)+
  stat_function(fun=demand, geom="line", size=2, color = "blue")+
    geom_label(aes(x=9,y=demand(9)), color = "blue", label="Demand", size = 4)+
  stat_function(fun=supply, geom="line", size=2, color = "red")+
    geom_label(aes(x=9,y=supply(9)), color = "red", label="Supply", size = 4)+
...
7. **In your own words, explain what an externality is. Give an example, and draw a graph — either for a negative externality or a positive externality. Explain _several_ possible ways that the problem can be solved.**

---

An externality is when an action or a choice undertaken by one party creates an external benefit or cost imposed on another party, and the decisionmaker is not internalizing this benefit or cost. The tradeoff that they make does not reflect the true social costs or benefits because the price does not fully capture them. 

An example of a negative externality is pollution. A producer chooses how much of a desired product to produce based on their marginal private cost and the marginal social benefit, at point A. However, a byproduct of their output is pollution, which creates an external cost that is not captured by the marginal private cost, making this lower than the true social cost. 

The solution to the externality is to internalize it within the prices that decisionmakers face. One possible solution is to simply tax the producer the amount of the external cost, making the marginal private cost and the marginal social cost the same. Another is to recognize that the source of the problem may be missing property rights, and to create and enforce property rights, and allow private parties to bargain. 

\clearpage

8. **In your own words, explain what a public good is, and give an example. Explain why markets face problems providing public goods.**

---

A public good is a good that is nonrivalrous and nonexcludable. By nonrivalrous, we mean that one person’s consumption does not physically prevent others from consuming the good simultaneously. By nonexcludable, we mean that no person owns the good and can legally exclude others from consuming the good.

An example would be national defense: all persons within a country enjoy protection from the military simultaneously, and nobody “owns” protection to be able to legally prevent persons from being protected.

Markets fail to adequately provide public goods because public goods create a *free rider problem*, since people can enjoy the good without paying for it, there is no incentive for individuals to contribute to financing or upkeeping the public good. Thus, it will not be profitable for private actors to produce the good.

---

9. **In your own words, what is the purpose of property rights?**

---

The primary purpose of property rights is to internalize externalities and avoid the tragedy of the commons by creating clear rules and responsibilities for how people are to use resources. 

By granting someone the right to exclude others from using property, it maintains an incentive for the owner to not deplete the resource in fear of others’ overuse, and also creates an incentive for the owner to improve the value of the resource — since they can earn the benefits. 

Additionally, it links ownership and responsibility by legally creating a libaility for the owner if the property should harm another person (create an external cost), thus creating another incentive for property owners to take care of their property such that it does not harm others.
...
10. The Ministry of Tourism in the Republic of Palau estimates that the monthly supply and demand for its scuba diving tours are: 
$$\begin{aligned}
	q_S&=30p-2000\\
	q_D&=6000-20p\\	
	\end{aligned}$$
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  stat_function(fun=demand, geom="line", size=2, color = "blue")+
    geom_label(aes(x=5000,y=demand(5000)), color = "blue", label="Demand", size = 4)+
  stat_function(fun=demand_2, geom="line", size=2, color = "blue")+
    geom_label(aes(x=5000,y=demand_2(5000)), color = "blue", label="Demand'", size = 4)+
  stat_function(fun=supply, geom="line", size=2, color = "red")+
    geom_label(aes(x=9000,y=supply(9000)), color = "red", label="Supply", size = 4)+
  geom_segment(x=2800, xend=2800, y=0, yend=160, size=1, linetype="dotted")+
  geom_segment(x=0, xend=2800, y=160, yend=160, size=1, linetype="dotted")+
  ...
  geom_label(x=2800,y=80,label="q*=2800")+
  geom_label(x=1400,y=160,label="p*=$160")+
  geom_label(x=3400,y=70,label="q'=3400")+
...
11. **The market for hotel rooms in a small town is characterized by the following equations:**
